function Xstruct = GLMinitSpikeHistDesignMat(gg,Xstruct)
% Xstruct = initfit_sphistDesignMat(gg,Xstruct)
%
% DESCRIPTION
%
%  Sets parameters relating to optimization of spike-filter terms in the 
%  point process GLM and inserts them into design matrix structure
%  'Xstruct'
%
...
for jcpl = 1:nCoupled
   % column indices
   inds = nh+nh2*(jcpl-1)+1:nh+nh2*jcpl;
   % insert into design matrix
   Xsp(:,inds) = spikefilt(full(double(gg.sps2(:,jcpl))),gg.ihbas2); 
end
